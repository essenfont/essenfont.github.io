...
export function initEasterEggs(): void {
  ...
  const TYPE_TRIGGERS: Record<string, () => void> = {
    ...
    'unicode': () => { flashOverlay('var(--ef-accent)', '99.79%'); confetti(); },
    ...
    'all': () => { flashOverlay('var(--ef-accent)', 'ALL OF UNICODE'); toast(TRIVIA[Math.floor(Math.random()*TRIVIA.length)], '★'); },
    '100%': () => { flashOverlay('var(--ef-accent)', '99.79%'); },
